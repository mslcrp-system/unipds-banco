-- ============================================================
-- insert_charges_from_raw: popular charges.comissao_coprodutor
--
-- A coluna comissao_coprodutor estava 100% vazia (0/14.386) porque o
-- ETL lia a chave errada ('Comissão Coprodutor'); o campo real do XLSX
-- e 'Valor comissão co-produtor'. Essa comissao e a "Taxa Secretaria"
-- do faturamento gerencial (secretaria academica = co-produtor na
-- Voomp) — split de pagamento que NAO chega na Unipds, logo entra no
-- calculo do valor liquido (e nao e tributado).
--
-- FORMATO do campo: "label: valor", podendo ter VARIOS co-produtores
-- separados por ';'. Ex.: "Pos Graduação: 0.00;Pos Graduação: 578.67".
-- Parser: split em ';', pega o numero apos cada ':', SOMA. Vazio -> 0.
--
-- Unica mudanca vs versao anterior: a extracao de comissao_coprodutor
-- no CTE base (de chave errada -> parser correto). Restante identico.
-- Aditivo: a coluna estava vazia e ninguem a lia.
-- ============================================================
...
BEGIN
    WITH base AS (
        SELECT
            ...
            COALESCE((
                SELECT sum(NULLIF(regexp_replace(trim(part), '^.*:\s*', ''), '')::numeric)
                FROM unnest(string_to_array(rl.payload->>'Valor comissão co-produtor', ';')) AS part
                WHERE trim(part) <> ''
