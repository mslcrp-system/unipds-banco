...
--
-- Segunda sub-funcao do ETL novo. Consolida raw_lines em
-- unipds.products via chave natural (tenant_id, voomp_produto_id).
--
-- Politicas paralelas a upsert_students_from_raw:
--   - Ultimo valor nao-nulo por campo
--   - Sem normalizacao de case (nomes de produto sao preservados)
...
DECLARE
    ...
    v_skipped     bigint := 0;
BEGIN
    -- ETAPA 1: ID Produto invalido -> inbox
...
COMMENT ON FUNCTION unipds.upsert_products_from_raw(text) IS
  'Consolida raw_lines em products. UPSERT via UNIQUE(tenant_id, voomp_produto_id). Ultimo valor nao-nulo por campo. Produto sem ID vai para raw_lines_skipped.';
